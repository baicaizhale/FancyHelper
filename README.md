...
- **🧠 强力 AI 驱动**：接入 CloudFlare Workers AI，默认使用 `@cf/moonshotai/kimi-k2.5` 大模型，理解意图更精准。
...
- **🐛 云端错误上报**：自动收集错误日志和配置文件并上报云端，帮助开发者改进插件（可配置关闭）。
- **📦 库管理命令**：支持通过 `/fancy lib install protocollib` 命令自动下载安装 ProtocolLib 依赖。
...
2. 推荐安装 [ProtocolLib](https://www.spigotmc.org/resources/protocollib.1997/) 插件以获得完整功能。你也可以在安装 FancyHelper 后使用 `/fancy lib install protocollib` 命令自动下载安装。
3. 重启服务器以生成默认配置文件。
...
插件内置了云端错误上报功能，帮助开发者改进插件。默认启用，可在配置文件中调整：
...
**库管理命令**：
- `/fancy lib install protocollib` - 自动下载并安装 ProtocolLib 插件（需要 OP 权限）
- 如果插件目录已存在 ProtocolLib 文件，会提示重启服务器加载
- 下载完成后需要重启服务器才能生效
...
**解决方案**：安装 [ProtocolLib](https://www.spigotmc.org/resources/protocollib.1997/) 插件以获得完整体验。你可以使用 `/fancy lib install protocollib` 命令自动下载安装（需要 OP 权限）。
