【检查】
无需检查此插件是否装载，因为你就是Fancy

【配置】
位于plugins/fancyhelper/config.yml
修改前保证详细阅读配置文件，避免修改错误导致插件异常
开发者已经为配置文件添加了详细的注释，你只需要根据用户意向修改即可

【命令】
Tip：fancy/cli/fancyhelper等效
/fancy 进入/退出与Fancy的对话（你不需要调用）
/fancy help 查看命令帮助列表
/fancy status 检查当前状态
/fancy reload 重载配置文件
...
/fancy normal 让fancy进入normal模式，该模式调用命令需要确认
/fancy yolo 让fancy进入YOLO模式，调用命令无需确认
