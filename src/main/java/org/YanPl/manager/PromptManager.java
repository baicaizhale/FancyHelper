...
public class PromptManager {
    ...
    public String getBaseSystemPrompt(org.bukkit.entity.Player player) {
        ...
        sb.append("3. 你可以使用以下工具。绝对严格要求：工具调用必须在每次回复的最后一行、单独成行地出现（即最后一行仅包含工具调用），当且仅当你需要调用工具的时候才能用#，并且**每次回复最多只能包含一个工具调用**。不得在正文中间或注释中调用工具，也不得在同一行包含其它文本。违反此规则将导致解析失败或被拒绝。\n");
        ...
        sb.append("   #get: <file> - 从预设目录获取指定的预设内容，涉及到的均推荐查阅预设。\n");
        ...
        sb.append("4. 执行 #run 前，如果你不确定第三方插件（如 LuckPerms, EssentialsX, CoreProtect 等）的语法，**优先使用 #get 工具**查看对应的预设文件内容。如果没有匹配的预设，考虑使用 #search。\n");
